...
class coarse_ext_parm_reproj_err {
public:                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             
	coarse_ext_parm_reproj_err(dlib::full_object_detection &observed_points_, bfm &model_) 
	: observed_points(observed_points_), model(model_) {}
	
    template<typename T>
	bool operator () (const T* const ext_parm, const T* const scale, T* residuals) const {
		const dlib::matrix<double> _fp_shape = model.get_fp_current_blendshape();

		T ext_parm_[6] = { T(0) };
		ext_parm_[0] = ext_parm[0];
		ext_parm_[1] = ext_parm[1];
		ext_parm_[2] = ext_parm[2];
		const dlib::matrix<T> fp_shape = transform(ext_parm_, _fp_shape);

		for(int i=0; i<N_LANDMARK; i++) {
			T u = scale[0] * fp_shape(i*3) / fp_shape(i*3+2) + scale[2];
			T v = scale[1] * fp_shape(i*3+1) / fp_shape(i*3+2) + scale[3];
			residuals[i*2] = T(observed_points.part(i).x()) - u;
			residuals[i*2+1] = T(observed_points.part(i).y()) - v;		
		}
		return true;
	...
	static ceres::CostFunction *create(dlib::full_object_detection &observed_points, bfm &model) {
		return (new ceres::AutoDiffCostFunction<coarse_ext_parm_reproj_err, N_LANDMARK * 2, N_EXT_PARM, 4>(
			new coarse_ext_parm_reproj_err(observed_points, model)));
	}
