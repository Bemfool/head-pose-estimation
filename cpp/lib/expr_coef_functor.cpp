#include "expr_coef_functor.h"

ExprNumericCostFunctor::ExprNumericCostFunctor(full_object_detection _shape) : shape(_shape) {} 

bool ExprNumericCostFunctor::operator()(const double* const x, double* residual) const {
	/* Init landmarks to be transformed */
	std::vector<point3f> landmarks3d(LANDMARK_NUM);
	double reg_term = get_landmarks_by_expr(x, landmarks3d);
	...
	for(int i=0; i<LANDMARK_NUM; i++) {
		...
		residual[i] = sqrt(tmp1 * tmp1 + tmp2 * tmp2) + 0.008 * reg_term;
	}
	return true;
}
